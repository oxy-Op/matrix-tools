...
function addCards(num, dimension = localStorage.getItem('dimension') == 'twice' ? 2 : 3) {
...
function generateCard(id, dimension) {
    const card = $('<div>', { class: 'card mb-md-0 mb-4', style: 'width: 200px;' });
    ...
    const h5 = $('<h5>', { class: 'text-center', text: `Matrix ${id}` }).appendTo(cardFooter);
...
function generateSign() {
    const div = $('<div>', { class: 'sign' });
}


function changeDimension(dimension) {
    const id = (localStorage.getItem('matrix')) ? parseInt(localStorage.getItem('matrix')) : 2
    if (dimension === 'twice') {
        addCards(id, 2)
